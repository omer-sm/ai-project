...
x = np.arange(-1600,1600, 5).reshape(1,640)
x = np.concatenate((x, x**3+20), axis=0)

gen = DLModel()
gen.add(DLLayer("", 256, (2,), "leaky_relu", "He", 6e-5, "rmsprop"))
gen.add(DLLayer("", 8, (256,), "vae_bottleneck", "He", 6e-5, "rmsprop", samples_per_dim=4))
gen.add(DLLayer("", 64, (16,), "leaky_relu", "He", 6e-5, "rmsprop"))
l = DLLayer("", 2, (64,), "leaky_relu", "Xavier", 6e-5, "rmsprop")
l.leaky_relu_d = 1.
gen.layers[1].leaky_relu_d = 1.
gen.add(l)
gen.compile("squared_means_KLD")
discrim = DLModel()
discrim.add(DLLayer("", 8, (2,), "leaky_relu", "Xavier", 6e-5, "rmsprop"))
discrim.add(DLLayer("", 16, (8,), "leaky_relu", "Xavier", 6e-5, "rmsprop"))
discrim.add(DLLayer("", 2, (16,), "leaky_relu", "Xavier", 6e-5, "rmsprop"))
discrim.add(DLLayer("", 1, (2,), "leaky_relu", "Xavier", 6e-5, "rmsprop"))
discrim.compile("squared_means")
gan = DLVAEGANModel("", gen, discrim)
gan.compile()
gan.train(x, 1500, 1, 5, 128)
plt.plot(x[0], x[1])
y = gan.generate(100)
plt.scatter(y[0], y[1])
plt.show()
